...
function createWorld() {
    ...
    for (let i = 0; i < rows; i++) {
        ...
        for (let j = 0; j < cols; j++) {
            ...
            cell.setAttribute('id', i + '_' + j); // adding and id so we know where this cell is located. 0_0 means left hand corner
            cell.setAttribute('class', 'dead'); // adding class to each cell so we can later change this. initial all cell will be dead until the user defines a patern.
            cell.addEventListener('click', cellClick); // listen to the click to change the initial state
            tr.appendChild(cell);
        }
        tbl.appendChild(tr);
    }
